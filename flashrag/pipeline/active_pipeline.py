...
from memsearcher import ReCall, MemSearcher
...
class ReCallPipeline(BasicPipeline):
    ...
    def __init__(self, config, retriever=None, generator=None):
        super().__init__(config, prompt_template=prompt_template_dict['re_call_template_sys'])
        ...
        self.re_call = self.engine_cls(model_url, executor_url, max_turns)
        self.max_turns = max_turns
        self.lengths = [0 for _ in range(max_turns)]
        self.nums = [0 for _ in range(max_turns)]
        self.peak = [0 for _ in range(max_turns)]
    ...
    def run_item(self, item):
        try:
            response, lengths = self.re_call.run(env=wikipedia_search_env.replace('<search-url-placeholder>', self.config["remote_retriever_url"]), func_schemas=wikipedia_search_schemas, question=item.question, tokenizer=self.tokenizer)
        except Exception as e:
            print(f"Error in re_call.run: {e}")
            item.update_output("final_response", "")
            item.update_output("pred", "")
            return
        for i, length in enumerate(lengths):
            self.peak[i] = max(self.peak[i], length)
            self.lengths[i] += length
            self.nums[i] += 1
        item.update_output("turns", len(lengths))
        item.update_output("ktokens", lengths[-1] // 1000)
...
class MemSearcherPipeline(ReCallPipeline):
    engine_cls = MemSearcher
