...
class TokenTemplate:
    ...
    def init(self, tokenizer):
        ...
        self.token_sections: list = []  # Store tokenized text sections as LongTensors
        self.last_section = None  # Last section as LongTensor
        ...
        for i, part in enumerate(parts[:-1]):
            if i % 2 == 0:  # Even index, non-{} part
                ...
                self.token_sections.append(tokens)
        ...
        self.last_section = tokenizer.encode(parts[-1], add_special_tokens=False)
    ...
    def length(self) -> int:
        ...
        total = sum(len(section) for section in self.token_sections)
        # total += self.last_section.numel()
        total += len(self.last_section)
        return total

    def format(self, **kwargs: dict[str, torch.LongTensor | list[int] | np.ndarray]):
        """
        ...
        for i, k in enumerate(self.keywords):
            ...
            formatted_parts = formatted_parts + self.token_sections[i]
            # formatted_parts.append(kwargs[k])
            formatted_parts = formatted_parts + kwargs[k]
        # formatted_parts.append(self.last_section)
        formatted_parts = formatted_parts + self.last_section
...
def reverse_indices(tensor):
    ...
    assert len(unique) == len(tensor), f"Your input tensor has duplicated elements. tensor={tensor.tolist()}"
