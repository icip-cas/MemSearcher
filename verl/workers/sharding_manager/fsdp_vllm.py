...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def postprocess_data(self, data: DataProto) -> DataProto:
        ...
        # if 'sample_index' not in data.batch:
        return data.chunk(chunks=self.tp_size)[self.tp_rank]

        # sample_index = data.batch['sample_index']
        # # print(self.tp_rank, sample_index, data.batch['final_mask'])
        # n_traj = sample_index.max().item() + 1
        # mini_n_traj = n_traj // self.tp_size
        # lower_bound = mini_n_traj * self.tp_rank
        # upper_bound = mini_n_traj * (self.tp_rank + 1)
        # mask = (sample_index >= lower_bound) & (sample_index < upper_bound)
        # indices = torch.nonzero(mask, as_tuple=True)[0]  # 得到满足条件的索引
        # return_data = data[indices]
        # # return_data.batch['sample_index'] -= lower_bound
        # # print(self.tp_rank, return_data.batch['sample_index'], return_data.batch['final_mask'])
        # return return_data
