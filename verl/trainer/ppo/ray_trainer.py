...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            valid = len(test_gen_batch)
            # test_output_gen_batch_padded = self.actor_rollout_wg.generate_sequences(test_gen_batch_padded)
            from recurrent.utils import final_batch
            test_gen_batch_padded.batch['sample_index'] = torch.arange(len(test_gen_batch_padded))
            test_output_gen_batch_padded = self.actor_rollout_wg.generate_sequences(test_gen_batch_padded)
            test_output_gen_batch = final_batch(test_output_gen_batch_padded, test_output_gen_batch_padded.batch['final_mask'], test_output_gen_batch_padded.batch['sample_index'])

            # unpad
            # test_output_gen_batch = unpad_dataproto(test_output_gen_batch_padded, pad_size=pad_size)
            mask = test_output_gen_batch.batch['sample_index'] < valid
            indices = torch.nonzero(mask, as_tuple=True)[0]
            test_output_gen_batch = test_output_gen_batch[indices]
    ...
    def init_workers(self):
        ...
        # from recurrent.impls.memory import MemoryConfig
        # self.recurrent_config = MemoryConfig(
        #     # context_key="my_context",
        #     max_prompt_length=self.config.data.max_prompt_length,
        #     # chunk_size=256,
        #     max_memorization_length=self.config.data.max_response_length,
        #     # max_chunks=8,
        #     max_final_response_length=self.config.data.max_response_length,
        #     # style="concise"
        # )
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        gen_batch.batch['sample_index'] = torch.arange(len(gen_batch))
                        # gen_batch_output, final_mask, sample_index = self.generation_manager.run_llm_loop(gen_batch, timing_raw)
                        try:
                            gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch)
                        except Exception as e:
                            print(f"Error during generating sequences: {e}")
                            continue
                        gen_batch_output.non_tensor_batch['uid'] = batch.non_tensor_batch['uid'][gen_batch_output.batch['sample_index']]
                        metrics["train/average_turns"] = len(gen_batch_output) / len(gen_batch)
                        _, counts = torch.unique(gen_batch_output.batch['sample_index'], return_counts=True)
                        metrics["train/max_turns"] = torch.max(counts).item()
                        # print(gen_batch)
                        # print(gen_batch_output.batch['sample_index'], gen_batch_output.batch['final_mask'])
                        # repeat to align with repeated responses in rollout

                        assert gen_batch_output.batch['final_mask'].sum().item() == len(batch.batch), \
                    ...
                    if self.config.trainer.balance_batch and False:
                    ...
                    with _timer('reward', timing_raw):
                        ...
                        try:
                            ...
                            from recurrent.utils import final_batch
                            # print(batch.batch['sample_index'])
                            # print(batch.batch['final_mask'])
                            reward_batch = final_batch(batch, batch.batch['final_mask'], batch.batch['sample_index']).union(original_batch)
                            reward_result = self.reward_fn(reward_batch, return_dict=True, curr_save_path=os.path.join(self.config.trainer.rollout_save_path, f'train_{self.global_steps}.jsonl'))
                    ...
                    with _timer('adv', timing_raw):
                        advantage_scalar = compute_1D_grpo_advantage(token_level_rewards=reward_tensor, 
                                                                    ...
                                                                    use_adv=True)
                        advantage_scalar = advantage_scalar[batch.batch['sample_index']]
                        ...
                        batch.batch['token_level_scores'] = reward_tensor[batch.batch['sample_index']]
